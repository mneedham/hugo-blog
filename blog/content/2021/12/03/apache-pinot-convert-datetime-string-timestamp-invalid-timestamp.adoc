...
docker exec \
  ...
  -jobSpecFile /config/new-blog/job-spec.yml
----
...
curl 'http://localhost:9000/tables/dates_OFFLINE' \
 ...
 --data-binary "@config/new-blog/table-transform.json"
----
...
----
docker exec -it manual-pinot-controller bin/pinot-admin.sh AddTable   \
  -tableConfigFile /config/new-blog/table-transform.json   \
  -schemaFile /config/new-blog/schema-transform.json -exec
----
