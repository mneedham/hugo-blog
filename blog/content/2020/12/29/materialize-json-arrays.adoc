...
I learnt from https://twitter.com/frankmcsherry[Frank McSherry^] that it's actually a feature of SQL that the `ORDER BY` clause isnt' inherited from the view.

 so I guess we may as well not have that line in the query that we used in the materialized view.
