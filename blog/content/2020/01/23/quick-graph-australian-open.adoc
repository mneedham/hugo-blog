...
CALL apoc.periodic.iterate(
  ...
  "WITH 'https://raw.githubusercontent.com/JeffSackmann/tennis_wta/master/wta_matches_' AS base, year
...
// Only get the longest path of NEXT_MATCH relationships that includes all matches played by the winner
...
RETURN path, [node in nodes(path) WHERE node:Match | [p = (p1)-[:WINNER]->(node)<-[:LOSER]-(p2) | p]];
----
...
CALL apoc.periodic.iterate(
  ...
  "WITH 'https://raw.githubusercontent.com/JeffSackmann/tennis_wta/master/wta_matches_' AS base, year
   ...
   WITH row, split(row.score, ' ') AS rawSets WHERE row.tourney_name = 'Australian Open'
   WITH row, rawSets, 
        [set in rawSets | apoc.text.regexGroups(set, \"(\\\\d{1,2})-(\\\\d{1,2})\")[0][1..]] AS sets,
...
image::{{<siteurl>}}/uploads/2020/01/aus-open-graph-sets.png[title="Sample of the Graph with sets added"]
...
RETURN winner.name AS winner, t.year AS year, size(matches) AS count, size(apoc.coll.flatten(sets)) AS sets,  matches
ORDER BY count DESC
LIMIT 5
----
